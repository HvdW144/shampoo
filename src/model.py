...
class Model:
    ...
    def __init__(self, disk=None, species=["H2O", "CO", "CO2", "CH4", "NH3", "H2S"], disk_folder="../ShampooBackground",
        ...
        self.mrat = float(
            self.paraDict["rat_erode"])  # mass ratio of colliders where erosion and fragmentation transition
    ...
    def loadPara(self, parameter_folder=None):
        ...
        paraDict = pd.read_csv(path + "shampooInput.csv", header=None, index_col=0, usecols=(0, 1), squeeze=True,
    ...
    def initPara(self, filepath):
        ...
        self.para["a_settle"] = float(
            self.disk.parameters["A_SETTLE"])  # the turbulent viscosity/dust settling parameters
    ...
    def initDensities(self, prodimo=False):
        ...
        for m in range(M):
            ...
            N = 2 * sig2D / (np.sqrt(
                2 * np.pi) * Hp * r * self.auTOm)  # We multiply as sigmaa is only 1/2 of the total surface
            # density
            mda = N * np.exp(expArg)
    ...
    def stickingFactor(self, t, r, z, iceName, dustT=None):
        ...
        tanhArg = self.para["betaStick"] * (
                    dustT - self.para["gammaStick"] * self.para["Eads" + iceName] / self.para["kB"])
        ...
        if self.legacyIce:
            if (self.monomer.iceTot_sol)[-1] > 0:
                if (self.monomer.ice_sol[iceName])[-1] > 0:
                    iceTot = 0
                    for n in range(self.iceNum):
                        iceTot += (self.monomer.ice_sol[self.disk.iceList[n]])[-1] / self.para[
                            "m" + self.disk.iceList[n]]
        ...
        if t_in == 0:
            ...
            for cond in self.trackListFun:
                if cond == "St":
                    ...
                    self.monomer.sec_sol[cond].append(
                        (self.monomer.t_sol[-1] - self.monomer.t_sol[-2]) * self.sTOyr * 1e3)
            if self.trackice:
                for species in self.disk.iceList:
    ...
    def determineCollisionOutcome(self, t, r, z, size=None):
        ...
        if fragmentation:  # if fragmentation we need to pick a new, smaller size of our home aggregate
            ...
            self.monomer.homeAggregate.prop["mAgg"] += self.monomer.homeAggregate.prop["mCol"] * \
                                                       self.monomer.homeAggregate.prop["NCol"]
            message = "coagulation with {:.1e} particle(s) of size {:.1e} m".format(
                self.monomer.homeAggregate.prop["NCol"], self.monomer.homeAggregate.prop["sCol"])
            outcome = "coagulation"
            interaction_id = 1
    ...
    def doIceEvolutionMyOwn(self, r_in, z_in, t_in):
        ...
        for n in range(self.iceNum):
            ...
            self.monomer.ice_sol[self.disk.iceList[n]].append(
                max([0, iceNew]))  # at the end we add the new ice solution
        ...
        if self.pisoBenchmark:
            ...
            self.monomer.prop["sMon"] = (3 * self.monomer.prop["mMon"] / (
                        4 * np.pi * self.monomer.prop["rho_mat"])) ** (1 / 3)
        ...
        if self.trackice:
            ...
            if self.store == 0:
                ...
                self.monomer.sec_sol["ratN/O"] = self.monomer.ele_sol[:, 2] / self.monomer.ele_sol[:, 3] * (
                            15.9994 / 14.0067)
                self.monomer.sec_sol["ratS/O"] = self.monomer.ele_sol[:, 4] / self.monomer.ele_sol[:, 3] * (
                            15.9994 / 32.06)
